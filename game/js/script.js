let pno = prompt("Enter the number of players") // player number
pno = parseInt(pno)
...
class game{
    addBus(name,color,pnum){ // pnum = player number
        ...
        <div style="display:flex;justify-content:center;">
            <p class="info current">Current Distance => <p id="current-p${pnum}">00</p>KM</p>
        </div>
        </div>
        `;
    }
    addBtn(color){
         ...
         btn.innerHTML = `
                <div class="dice-btn" style="background-color:${color};" onclick="g.rollDice('${color}')">Roll dice</div>
            `;
    }
    rollDice(color){
        ...
        let fakeRols = setInterval(()=>{
            ...
            <div class="dice-roll">
                  <img src="assets/dice/dice_${diceNo}.png" style="background-color:${color};">
            </div>
            `
            diceVal = diceNo;
        ...
        setTimeout(()=>{
            ...
            if (targetScore>100){
                ...
                setTimeout(()=>{
                    ...
                    rollBtn.style.display = "block";
                    rollBtn.scrollIntoView()
                },3000)// 2 second beacuse the parent timeout will run for 3 seconds
            }
        },3000)    
    }
}
...
for (let i = 1;i<pno+1;i++) {
    ...
    let color = prompt("Enter the playing color of player " + i)
    pnameArr.push(name)
    pcolorArr.push(color)
}
